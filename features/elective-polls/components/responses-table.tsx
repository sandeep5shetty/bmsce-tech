...
export interface PollResponseRow {
  ...
  optionLabel: string;
  submittedAt: string;
  /** Set when this response was submitted through a selective reopen grant
   * (the poll was closed at the time) — the remark that authorized it. */
  reopenGrant: {
    remarks: string;
    grantedAt: string;
...
export function ResponsesTable({
  ...
  >("all");
  const [detailsTarget, setDetailsTarget] = useState<PollResponseRow | null>(
    null,
  );
  ...
  const hasReopened = useMemo(
    () => responses.some((r) => r.reopenGrant),
    [responses],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return responses.filter((r) => {
      ...
      if (sectionFilter !== "all" && r.studentSection !== sectionFilter)
        return false;
      if (reopenFilter === "reopened" && !r.reopenGrant) return false;
      if (reopenFilter === "not-reopened" && r.reopenGrant) return false;
      if (!q) return true;
      const haystack =
        `${r.studentName ?? r.userName ?? ""} ${r.studentUsn ?? ""} ${r.email}`.toLowerCase();
      return haystack.includes(q);
    });
  }, [responses, search, optionFilter, sectionFilter, reopenFilter]);
  ...
  function exportCSV() {
    ...
    const rows = filtered.map((r) => [
      ...
      r.reopenGrant?.remarks ?? "",
      r.reopenGrant?.grantedByName ?? "",
    ]);
  ...
  return (
    <>
      {detailsTarget?.reopenGrant && (
        <ReopenGrantDetailsDialog
          open={!!detailsTarget}
          onOpenChange={(open) => !open && setDetailsTarget(null)}
          studentName={
            detailsTarget.studentName ??
            detailsTarget.userName ??
            detailsTarget.email
          }
          remarks={detailsTarget.reopenGrant.remarks}
          grantedAt={detailsTarget.reopenGrant.grantedAt}
          grantedByName={detailsTarget.reopenGrant.grantedByName}
        />
      )}
      <Card>
                      ...
                      <TableCell>
                        {r.reopenGrant && (
                          <button
                            ...
                            aria-label={`View reopen details for ${r.studentName ?? r.userName ?? r.email}`}
                          >
                            <Badge
                              variant="outline"
                              className="cursor-pointer gap-1 border-green-500/40 text-xs text-green-700 transition-colors hover:bg-green-500/10 dark:text-green-400"
                            >
