  ...
  if (variant === "card") {
    return (
      <div
        ...
        {...listeners}
        {...attributes}
        className="group bg-card hover:shadow-primary/5 hover:border-primary/30 relative flex cursor-grab overflow-hidden rounded-xl border transition-all duration-300 active:cursor-grabbing"
      >
...
export default function RandomPickerPage() {
  ...
  async function reseedStudents() {
    const confirmed = window.confirm(
      "Replace all students with the latest MCA 2025-27 roster? Your pick history will be cleared.",
    );
    if (!confirmed) return;
  ...
  useEffect(() => {
    localStorage.setItem(
      "rp_exclude_coordinators",
      String(excludeCoordinators),
    );
  }, [excludeCoordinators]);
  ...
  function isCoordinatorExcluded(student: Student) {
    return (
      excludeCoordinators &&
      isActivityCoordinator(student.usn) &&
      !alreadyPicked.has(student.id)
    );
  }
  ...
  function pickRandom(addToCurrent = false) {
    ...
    if (!n || n < 1) {
      toast.error("Enter a valid number (min 1)");
      return;
    }

    const available = students.filter(isPoolEligible);
    if (available.length === 0) {
      toast.error("All students picked. Reset to continue.");
      return;
    }
  ...
  const excludedStudents = students.filter(isExcludedFromPool).sort((a, b) => {
    ...
    if (aIsCoordinator !== bIsCoordinator)
      return aIsCoordinator - bIsCoordinator;
      ...
      <div className="container mx-auto mt-8 mb-32 max-w-6xl px-4 sm:px-6">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            MCA 1st yr Sec B — {students.length} students · {availableCount}{" "}
            available · {excludedCount} excluded
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
              ...
              <div className="flex flex-wrap items-end gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    onClick={reset}
                    disabled={
                      alreadyPicked.size === 0 && pickedStudents.length === 0
                    }
                  >
              ...
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <p className="text-muted-foreground text-xs">
                    Manually excluded students and activity coordinators. Use +
                    to restore picks to the pool.
                  </p>
                </CardHeader>
                <CardContent>
          ...
          <Card className="flex max-h-[calc(100vh-8rem)] flex-col gap-0 shadow-md lg:sticky lg:top-4">
            <CardHeader className="shrink-0 space-y-4 pb-2">
              ...
              <label className="hover:bg-muted/60 bg-background flex cursor-pointer items-center gap-2.5 rounded-lg border px-3 py-2 transition-colors">
                <input
